defmodule IslandsEngine.Game do
  ...
  def handle_call({:add_player_old, name}, _from, state) do
  ...
  def handle_call({:set_island_coordinates_old, player, island, coordinates}, _from, state) do
    state
    |> Map.get(player)
  ...
  # def handle_call({:guess_old, player, coordinate}, _from, state) do
  ...
  def old_start_link(name) when not is_nil(name) do
  ...
  @spec set_islands(atom | pid | {atom, any} | {:via, atom, any}, atom) :: any
  def set_islands(game, player) when is_atom(player) do
    GenServer.call(game, {:set_islands, player})
  end

  def guess_coordinate(game, player, coordinate) when is_atom(player) and is_atom(coordinate) do
